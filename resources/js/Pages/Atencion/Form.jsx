...
export default function AtencionForm({ auth, atencion, pacientes, medicamentos, materiales }) {
  const { data, setData, post, put, processing, errors } = useForm({
    ...
    medicamentos:
      atencion?.medicamentos_usados?.map((item) => ({
        id: item.medicamento.id,
        cantidad_usada: item.cantidad_usada,
      })) || [],
    materiales:
      atencion?.materiales_usados?.map((item) => ({
  ...
  // Sincronizar arrays con useForm
  useEffect(() => {
  ...
  useEffect(() => {
    const imc = calculateIMC(parseFloat(data.peso), parseFloat(data.talla));
    setData("IMC", imc);
  ...
  const handleSubmit = (e) => {
    ...
    setData("medicamentos", medicamentoFields.filter((m) => m.id && m.cantidad_usada));
    setData("materiales", materialFields.filter((m) => m.id && m.cantidad_usada));

    if (atencion) {
      put(route("atencion.update", atencion.id), { preserveScroll: true });
    } else {
      post(route("atencion.store"), { preserveScroll: true });
    }
  };
  ...
  const handleMedicamentoChange = (index, field, value) => {
    ...
    updated[index][field] = field === "id" ? parseInt(value) || "" : value;
